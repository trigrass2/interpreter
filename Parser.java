...
public class Parser {
	...
	private Statement readLines(ArrayList<String> lines) throws Exception {
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			} else if (isFunction(currentLine)) {
				String name = currentLine.replace("function", "");
				ArrayList<String> functionBlock = new ArrayList<String>();
				int nestCount = 1;
				String whileLine;
				while (nestCount != 0) {
					whileLine = lines.get(i+1);
					...
					functionBlock.add(whileLine);
					//System.out.println(whileLine + " (" + nestCount + ")");
					i++;
				}
				Function f = new Function(name, readLines(functionBlock));
				//block.add(f);
				functions.add(f);
			} else if (isCall(currentLine)) {
				String name = currentLine.replace("call", "");
				Call g = new Call(name);
				block.add(g);
	...
	private boolean isCall(String c) {
	...
	private boolean isFunction(String f) {
		if (f.contains("function") && isCall(f)) {
			return true;
		}
		return false;
	}
